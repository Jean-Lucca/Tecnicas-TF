...
async function mostrarCarros(carros) {
  ...
  carros.forEach((carro, index) => {
    ...
    const elemento_vin = elemento.querySelector("div small");
    elemento_vin.innerHTML = carro.vin;
    ...
    elementos_preco[2].innerHTML = "Desconto:    " + carro.desconto * 10 + "%";
...
function lerDatas() {
  const inputs_inicio = document.querySelectorAll('#inicioLocacao input[type="date"]');
  const inicio = new Date(inputs_inicio[0].value);

  const inputs_fim = document.querySelectorAll('#fimLocacao input[type="date"]');
  const fim = new Date(inputs_fim[0].value);
